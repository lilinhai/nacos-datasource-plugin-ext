...
public class ConfigInfoTagMapperByOracle extends AbstractOracleMapper implements ConfigInfoTagMapper
{
     ...
     */
    public ConfigInfoTagMapperByOracle()
    {
    ...
    public MapperResult findAllConfigInfoTagForDumpAllFetchRows(MapperContext context)
    {
        ...
        String innerSql = getDatabaseDialect().getLimitPageSqlWithOffset("SELECT id FROM config_info_tag  ORDER BY id ", startRow, pageSize);
    ...
    public MapperResult updateConfigInfo4TagCas(MapperContext context)
    {
        ...
        String sql = "UPDATE config_info_tag SET content = ?, md5 = ?, src_ip = ?,src_user = ?,gmt_modified = ?,app_name = ? " + "WHERE data_id = ? AND group_id = ? AND tenant_id = NVL(?,'"
                + NamespaceUtil.getNamespaceDefaultId() + "') AND tag_id = ? AND (md5 = ? OR md5 IS NULL OR md5 = '')";
        return new MapperResult(sql, CollectionUtils.list(content, md5, srcIp, srcUser, gmtModified, appName, dataId, groupId, tenantId, tagId, oldMd5));
    }
}
